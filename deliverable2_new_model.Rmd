...
#%>% mutate(research=(str_detect(comments, "research")|str_detect(comments, "Research"))&!str_detect(comments, "No research")&!str_detect(comments, "no research")&!is.na(comments))
...
In model without internation distribution, the mean of 
In model with interation, 
...
ggplot(grad_mod_int, aes(logit, value))+
  ...
  theme_bw() + 
  facet_wrap(~predictors_int, scales = "free_y")+labs(title = 'The Linearity of Independent Variables' )
...
ggplot(data_int, aes(index, .std.resid)) + 
  ...
  theme_bw() + labs(y = 'Deviance Residuals', x = 'Index', titles = 'Residuals Plot' )
# if standardized residual is greater than 3 -> Influential
data_int %>% 
  filter(abs(.std.resid) > 3)

#multicollinearity
car::vif(full_mod_int)

#Correlation matrix
library(Hmisc)

my_data1 <- grad[, c(8,11,17)]
my_data2 <- grad[, c(8,9,10,11)]


(rcorr(as.matrix(my_data1))) 
#This is for ugrad_gpa, gre_total gre_writing
...
# Pr(>Chi) for ugrad_gpa, GRE_Total, gre_writing and status are all smaller than siginificant level 0.05. While all the interaction effect is not signficant. Therefore, the anova table indicates that the main effect are significant.
