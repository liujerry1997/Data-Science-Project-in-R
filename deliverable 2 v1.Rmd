...
<<<<<<< HEAD
grad <- grad %>% 
  mutate(decision1 = (decision=="Accepted")) %>%
  filter(is_new_gre == TRUE & is.na(gre_quant)!= TRUE & is.na(gre_verbal)!= TRUE & is.na(gre_writing) != TRUE, ugrad_gpa <=4, is.na(ugrad_gpa) != TRUE) %>%
  filter(uni_name != "Purdue University")

full_mod <- glm(decision1 ~ ugrad_gpa+gre_verbal+gre_quant+gre_writing+status, data = grad, family = binomial)
full_mod_cb <- glm(decision1 ~ ugrad_gpa+gre_verbal^3+gre_quant^3++status, data = grad, family = binomial)
=======
...
>>>>>>> 19a3abffb54eaeeee13be9f79f7eb97a109dc275
null_mod <- glm(decision1 ~ 1,data = grad, family=binomial)

backwards = step(full_mod)
backwards_cb = step(full_mod_cb)
summary(backwards)
summary(backwards_cb)
...
# linearity
probabilities <- predict(full_mod_cb, type = "response")
grad_mod <- grad %>% 
...
ggplot(grad_mod, aes(logit, predictor.value))+
...
plot(full_mod_cb, which = 4, id.n = 3)
car::vif(full_mod_cb)
...
grad_mod <- (grad_mod %>%
  ...
  gather(key = "predictors", value = "pred", -logit))

ggplot(grad_mod, aes(logit, pred))+
  geom_point(size = 0.5, alpha = 0.5) +
  geom_smooth(method = "loess") + 
  theme_bw() + 
  facet_wrap(~predictors, scales = "free_y")
