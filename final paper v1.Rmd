...
```{r, include=FALSE}
#load packages plus dataset, also lots of old code from deliverable one
library(knitr)
library(tidyverse)
library(car)
library(Hmisc)
...
The dataset has official data of report from 2010 to 2015. The application report of two degrees, MS and PhD increase each year until 2015. The overall shape has a positive trend.
...
grad %>% filter(!is.na(status)) %>% 
  mutate(count = n()) %>% 
  ggplot() + stat_count(aes(x = status, y = ..prop.., group = TRUE)) + 
  labs(titles = "Density of Immigration Status") 
...
From the charts above, it seems that US based students tend to have higher acceptance rates than international students, and international students with US degree. The bar chart shows that, for American students and international students with a US degree, the number of getting accepted is higher than the number of getting rejected. However, for international students without a US degree, the number of acceptance is lower than the number of rejection. 
