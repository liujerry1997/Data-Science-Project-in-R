...
labs(x ="Year",
     ...
     caption = "Fig 1. Overall trend in the number of reported applications per year increasing")
```
...
grad %>% filter(!is.na(status)) %>% 
  ...
  labs(titles = "Frequency Distribution of Immigration Status",
       caption = "Fig 3. This figure shows the number of students for each immigration status category") 
```
...
grad %>%
  ...
  labs(title = "Admission Decision vs Immigration Status",
       caption = "Fig 4. This figure shows the distribution of admission results with respect to student status")
```
...
ggplot(aes(x = mean_GRE, y = mean_gpa)) + geom_point(aes(color = uni_name, alpha = 0.001)) +
  labs(titles = "Relationship between GPA and GRE Score",
       y = "GPA",
       x = "GRE Score",
       caption = "Fig 5. mean GPA and GRE for Top 10 CS program")
```
...
ggplot(grad_mod, aes(logit, value))+
  ...
  labs(titles = "Linearity of Numerical Variables",
       caption = "Fig 6. plots the linearity between the logit of outcome and the value of numerical varibales")
...
plot(full_mod, which = 4, id.n = 3,
     sub = "Fig 7. describes the cook's distance for all observations in the dataset")
...
ggplot(data, aes(index, .std.resid)) + 
  ...
  labs(titles = "Standardized Residuals",
       ...
       caption = "Fig 8. plots the distance between predicted value and residuals")
...
###Model With Interaction
 However, the variables in model without interactions are correlated with each other and we also suspect that there may be interactions between student status and scores. Therefore, we construct another model that includes interaction between the status and other variables. In addition, 
...
##Assumption 
To ensure the validity of this model, we follow the same methodology in assumptions in model without interaction.
...
ggplot(grad_mod_int, aes(logit, value))+
  ...
  labs(titles = "Linearity of Numerical Variables",
       caption = "Fig 9. plots the linearity between the logit of outcome and the value of numerical varibales")
...
ggplot(data_int, aes(index, .std.resid)) + 
  ...
  labs(titles = "Standardized Residuals",
       x = "index",
       y= "Standard Deviation",
       caption = "Fig 10. plots the distance between predicted value and residuals")
...
interaction.plot(x.factor     = wr$status,
                 ...
                 sub = "Fig 11.  Interaction Plot Base on information of top 10 Computer Science programs",
                 xlab="Student status",
...
(anova(full_mod_int))
```
The plot suggests that the effect of GRE writing is not consistent across all three groups of students.  For example, a writing score of 5 showed the greater mean probability of acceptance for American and international students with US Degree. for international student, a writing score of 3 gives the highest chance of acceptance. This suggests there may be a meaningful or significant interaction effect, but we will need to do a statistical test to confirm this hypothesis. 
...
grid.arrange(p1, p2, ncol = 2, bottom = "Fig 12. The boxplot of our models' outcome")
